...
- FloatingContainer
  ...
  - 只能放进垂直布局 (`Container` / `Popover` / `Dialog` / 根布局), 放进 `Row` 会抛 `ValueError`: 角落的横向对齐靠的是 `align-self`, 而 `Row` 的 flex 方向会与之打架.
...
- `TreeSelectDualPaneWithInput` 的 "Recent" 下拉: 列表被替换时, 内部 radio 会自己 adopt 最新一条 (`_auto_select`), 而这次 adopt 看起来跟"用户挑了一项"一模一样 —— 以前包装器会因此跑一遍 `_commit`, 于是"面板里勾一个文件夹"会把面板一起带走. 现在 `Recent` 用与 `_auto_select` 相同的判据预先记下它将 adopt 哪一项, 回传时跳过它: 列表刷新不再伪装成挑选, 而真·下拉挑选仍照常 `_commit` (文件夹 → 面板跳过去, 文件 → 加入选中). `TreeSelectWithInput` 已经不用这个下拉了: 它把 `PathInput.candidates` 打开 (`candidates=[]`), 盒子自己就是"最近路径"记录 —— 初值 `start_directory`, 之后每解析出一个路径就长一条 (字母序列出), 少了 `Recent` 那个按钮, 也少了上面这条 `_auto_select` 的弯路.
