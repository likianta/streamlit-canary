    ...
    with v3.Row('bottom'):
        ...
        with v3.Popover(':material/adjust:', help='Line calibration'):
            with v3.Container(width=540):
                with v3.Tabs(
                    ('One-line Calibration', 'Two-line Calibration')
                ) as tabs:
                    with next(tabs):
                        with v3.Row('bottom'):
                            with v3.Grid(
                                ...
                                with next(grid):
                                    v3.NumberInput(
                                        ...
                                        help=(
                                            """
                                        The Bias parameter is used to measure 
                                        the pulse offset caused by the routing 
                                        design on the PCB board. 

                                        Roughly speaking, you can take count of 
                                        each 3cm trace length as 1 point, and 
                                        considering the round trip, the trace 
                                        and the number of points need to be 
                                        multiplied by 2 respectively.

                                        For example: If the routing distance is 
                                        approximately 2.5 cm, then the *bias* 
                                        value should be `(2.5 * 2) / 3 = 1.67` 
